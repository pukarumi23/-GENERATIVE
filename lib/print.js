...
export default async function (m, conn = { user: {} }) {
    ...
    let chatName = chat ? (m.isGroup ? 'Concierto Virtual: ' + chat : 'Chat Privado con Fanático: ' + chat) : '';
...
if (/document/i.test(m.mtype)) console.log(`💙 ${m.msg.fileName || m.msg.displayName || 'Documento Virtual'}`)
else if (/ContactsArray/i.test(m.mtype)) console.log(`🌟 ${' ' || ''}`)
else if (/contact/i.test(m.mtype)) console.log(`✨ ${m.msg.displayName || ''}`)
else if (/audio/i.test(m.mtype)) {
const duration = m.msg.seconds
console.log(`🎤ㅤ(PTT ' : '🎵ㅤ('}AUDIO VIRTUAL) ${Math.floor(duration / 60).toString().padStart(2, 0)}:${(duration % 60).toString().padStart(2, 0)}`)
}
console.log()
}
let file = global.__filename(import.meta.url)
watchFile(file, () => {
    console.log(chalk.redBright("Update 'lib/print.js' - Sistema Virtual de Miku"))})
